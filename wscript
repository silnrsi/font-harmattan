...
# APs to omit:
OMITAP = '-o "_above _below _center _ring _through above below center ring through"'

for style in ('-Regular', ):  # '-Bold'):
    font(target=APPNAME + style + '.ttf',
        source='source/' + APPNAME + style + '.ufo',
        #ap = 'source/' + APPNAME + style + '.xml',		# autogenerated from UFO doesn't yet work.
        #ap = create(APPNAME + style + '.xml', 
        #    cmd('python /home/ubuntu/pysilfont/scripts/UFOexportAnchors.py -g ${SRC} ${TGT}',
        #        [ 'source/' + APPNAME + style + '.ufo' ])),  # hack
        version=VERSION,
